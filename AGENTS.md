...
- A long-lived native process gets slower at the same allocation-heavy work each time it repeats it (Bend 2.0.26/2.0.27: a board parse 0.2 s → 1.6 s over six runs, memory flat). Run big repeated parses in a child process (`backplane --plot`, src/server/plot.bend); `BACKPLANE_BENCH_RUNS=6 build/plot_bench FILE` shows it.
