...
- **Mechanical parts.** Agents model enclosures, brackets and assemblies with [FreeCAD](https://www.freecad.org), headless (install it yourself; Settings and the agents say where when it is missing). Parts live in the project's `mech/` folder as a Python script, `.FCStd` and STEP. `backplane --render mech/part.step` draws its iso, front, top and right views to `mech/renders/` with Backplane's own 3D rasterizer, nothing else to install. The Mech tab (window, web, phones) shows every part with its renders and opens it in 3D.
